...
def bootstrap(path):
    ...
    try:
        settings = imp.load_source(hashlib.md5(settings_file).hexdigest(), settings_file)
    except ImportError:
...
def get_config_identifiers(path, pattern, directory):
    settings_path = find_settings_path(path)
    config_dir = os.path.abspath(os.path.join(settings_path, pattern))
    configs = glob.iglob(config_dir)
    files = list(configs)
    if not files:
        sys.stderr.write("Your django application does not appear to have been setup with switchable configurations.\n")
        sys.exit(0)

    identifiers = map(lambda name: directory and os.path.basename(os.path.dirname(name)) or name.split('.', 1)[0], filter(lambda name: name.endswith(".py") and name not in ('__init__.py', 'base.py',), files))
    return [ (i + 1, x) for i, x in enumerate(identifiers) ]

def setup_environment(path, pattern='config/*.py', directory=False):
    IDENTIFIERS = get_config_identifiers(path, pattern, directory)
